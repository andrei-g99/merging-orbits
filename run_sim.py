...
for t in range(simulation_steps):
    ...
    for particle in particle_list:
        timestep_data.append(particle.position_history[-1].tolist() + particle.velocity_history[-1].tolist())
    simulation_data.append([t] + timestep_data)
...
with open('simulation_data.csv', 'w', newline='') as file:
    ...
    for i in range(len(particle_list)):
        header += [f'particle_{i}_pos_x', f'particle_{i}_pos_y', f'particle_{i}_pos_z',
                   f'particle_{i}_vel_x', f'particle_{i}_vel_y', f'particle_{i}_vel_z']
